...
func (cpu *CPUContext) executeLoad(instr Instruction) error {
	...
	if cpu.interfaceMode == 1 && instr.Address != [4]bool{} {
		// Pipeline режим - используем предвычисленный адрес
		addr = instr.Address
	} else {
		// Shell режим - вычисляем адрес из регистра
...
func (cpu *CPUContext) executeJump(instr Instruction) error {
	...
	if instr.Label != "" {
		// Работа с метками (одинаково в обоих режимах)
		...
		if cpu.interfaceMode == 1 && instr.Address != [4]bool{} {
			...
			// Shell режим - вычисляем из регистра
