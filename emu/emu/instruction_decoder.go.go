package main

type Instruction struct {
	OpCode    int
	Reg1      int
	Reg2      int
	Imm       [4]bool
	Label     string
	Address   [4]bool
	isImm     [4]bool
	isMemDest bool
	IsMemDest bool // true если назначение - память
	IsMemSrc  bool // true если источник - память
}

func DecodeInstruction(instr [4]bool) Instruction {
	op := boolToInt(instr[0])<<3 | boolToInt(instr[1])<<2 |
		boolToInt(instr[2])<<1 | boolToInt(instr[3])

	decoded := Instruction{
		OpCode: op,
		Reg1:   boolToInt(instr[2]),
		Reg2:   boolToInt(instr[3]),
	}

	// Для MOV с immediate-значением
	if op == OpMov && instr[0] {
		decoded.Imm = [4]bool{instr[1], instr[2], instr[3], false}
		decoded.Reg2 = -1
	}

	// Для команд работы с памятью и переходов в pipeline режиме
	if op == OpLoad || op == OpStore || (op >= OpJmp && op <= OpJc) {
		decoded.Address = [4]bool{false, instr[1], instr[2], instr[3]}
	}

	return decoded
}
