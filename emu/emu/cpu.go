...
func convertToInstruction(parts []string) (Instruction, error) {
	...
	// Определяем режим (pipeline или shell)
	...
	// Парсинг аргументов с учетом режима
	if len(parts) > 1 {
		// Первый аргумент
		if reg, err := parseArg(parts[1], opcode == OpMov, pipelineMode); err == nil {
			if reg.isReg {
		...
		// Второй аргумент
		if len(parts) > 2 {
			if reg, err := parseArg(parts[2], opcode != OpStore, pipelineMode); err == nil {
				if reg.isReg {
...
func parseArg(arg string, allowValue bool, pipelineMode bool) (parsedArg, error) {
	// Обработка регистра (r0-r3)
	...
	// В pipeline режиме разрешаем только регистры и строго форматированные значения
	if pipelineMode {
		// Попытка распознать immediate-значение (4 бита)
	...
	// В shell режиме более гибкий парсинг
	if allowValue {
		// Попытка распознать двоичное значение
		if len(arg) == 4 {
			var bits [4]bool
			valid := true
			for i, c := range arg {
		...
		// Попытка распознать число (0-15)
		if num, err := strconv.ParseInt(arg, 10, 16); err == nil && num >= 0 && num < 16 {
			var bits [4]bool
			for i := 0; i < 4; i++ {
				bits[i] = (num>>i)&1 == 1
			}
	...
	return parsedArg{}, fmt.Errorf("неверный аргумент: %s", arg)
}
...
func convertTextToInstruction(cmd string, args []string) (Instruction, error) {
	...
	pipelineMode := strings.Contains(cmd, "|") // Определяем режим работы
	...
	case "mov":
		...
		// Парсим назначение (регистр или память)
		if strings.HasPrefix(args[0], "[") && strings.HasSuffix(args[0], "]") {
			...
			instr.Address = intTo4Bits(reg) // Временное значение, реальный адрес будет вычислен при выполнении
		} else {
			// MOV в регистр: rX, src
		...
		// Парсим источник (регистр, память или immediate)
		if strings.HasPrefix(args[1], "[") && strings.HasSuffix(args[1], "]") {
			...
			instr.IsMemSrc = true
			instr.Address = intTo4Bits(reg) // Временное значение
			...
			// MOV с immediate-значением
			if pipelineMode {
				// В pipeline режиме только строгие 4 бита
				if len(args[1]) != 4 {
				...
				// В shell режиме принимаем числа или 4 бита
			...
			instr.Reg2 = -1 // Помечаем как immediate
		}
...
func (c *Command) String() string {
	opCodeNames := map[int]string{
		...
		OpMov:   "mov", // Добавлено
		OpCmp:   "cmp",
		OpLoad:  "load",  // Добавлено
		OpStore: "store", // Добавлено
		OpJmp:   "jmp",
		OpJz:    "jz",
		OpJnz:   "jnz",
		OpJc:    "jc",
